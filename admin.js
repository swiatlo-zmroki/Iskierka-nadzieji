// === BEZPIECZNA KONFIGURACJA FIREBASE ===
// Klucze są teraz wczytywane ze zmiennych środowiskowych Vercela
...
function initializeAppLogic() {
    ...
    document.getElementById('adminPanel').addEventListener('click', (e) => {
        if (e.target.matches('.delete-section-btn')) {
            if (confirm(`Na pewno chcesz usunąć sekcję "${e.target.dataset.name}" i wszystkie jej wpisy?`)) {
                const batch = db.batch();
                db.collection('entries').where('sectionId', '==', e.target.dataset.id).get().then(snap => {
                    snap.forEach(doc => batch.delete(doc.ref));
                    batch.delete(db.collection('_sections').doc(e.target.dataset.id));
                    batch.commit().then(loadSections);
                });
            }
        }
        if (e.target.matches('.delete-entry-btn')) {
            if (confirm('Na pewno usunąć ten wpis?')) {
                db.collection('entries').doc(e.target.dataset.id).delete().then(loadEntries);
            }
        }
        if(e.target.matches('.delete-help-btn')){
            if(confirm('Na pewno usunąć to miejsce?')){
                db.collection('miejsca_pomocy').doc(e.target.dataset.id).delete().then(loadHelpPlaces);
            }
        }
    });

    document.querySelector('button[onclick="addGalleryImage()"]').onclick = async () => {
    ...
    document.querySelector('button[onclick="addHelpPlace()"]').onclick = async () => {
    ...
    document.querySelector('button[onclick="addNewSection()"]').onclick = async () => {
    ...
    document.querySelector('button[onclick="addEntry()"]').onclick = async () => {
        const text = document.getElementById('entryText').value.trim();
        if (!text) return alert("Wpisz treść!");
        await db.collection('entries').add({ sectionId: document.getElementById('sectionSelect').value, title: document.getElementById('entryTitle').value.trim() || null, text, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
        document.getElementById('entryTitle').value = ''; document.getElementById('entryText').value = ''; alert('Wpis dodany!'); loadEntries();
    };
    
    document.querySelector('button[onclick="addSpark()"]').onclick = async () => {
    ...
    document.querySelector('button[onclick="addSongToPlaylist()"]').onclick = async () => {
