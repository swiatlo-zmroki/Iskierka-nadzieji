...
// Ta funkcja jest teraz sercem wszystkiego i zostanie wywołana tylko raz.
function initializeAppLogic() {
    ...
    async function loadSections() {
        ...
        sectionSelect.innerHTML = ''; editSectionSelect.innerHTML = ''; menuEditorDiv.innerHTML = '';
        snapshot.forEach(doc => {
            ...
            div.appendChild(span); div.appendChild(button);
    ...
    async function loadEntries() {
        ...
        snapshot.forEach(doc => {
            ...
            div.innerHTML = `<div class="font-semibold">${entry.title || '(bez tytułu)'}</div><p class="text-gray-700">${entry.text}</p>`;
            const button = document.createElement('button');
            button.className = 'text-red-600 hover:underline mt-2 text-sm';
            button.textContent = 'Usuń';
    ...
    async function loadHelpPlaces() {
        ...
        snapshot.forEach(doc => {
            ...
            const button = document.createElement('button');
            button.className = 'bg-red-500 text-white px-2 py-1 rounded text-sm flex-shrink-0';
            button.textContent = 'Usuń';
            button.onclick = () => deleteHelpPlace(place.id);
            div.appendChild(button);
    ...
    async function addGalleryImage() {
        ...
        await db.collection('gallery').add({ url, description: galleryDescInput.value.trim() || null, uploadedAt: firebase.firestore.FieldValue.serverTimestamp() });
    ...
    // Znajduje przyciski po ich `onclick` i przypisuje im funkcje
    document.querySelector('button[onclick="addGalleryImage()"]').onclick = addGalleryImage;
    document.querySelector('button[onclick="addHelpPlace()"]').onclick = addHelpPlace;
    document.querySelector('button[onclick="addNewSection()"]').onclick = addNewSection;
    document.querySelector('button[onclick="addEntry()"]').onclick = addEntry;
    document.querySelector('button[onclick="addSpark()"]').onclick = addSpark;
    document.querySelector('button[onclick="addSongToPlaylist()"]').onclick = addSongToPlaylist;
